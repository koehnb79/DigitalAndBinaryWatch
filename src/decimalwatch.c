...
static char* getBinary(int bin) {
    int factor=0;
    while (pow(2, factor)<=bin)
        factor++;

    factor=16;
    char *binary=(char*)malloc(factor+1);

    int pos=0;
    while ((--factor)>=0) {

        int blue=pow(2,factor);
        int thing=blue & bin;
...
static int getDecimal(int hour, int minute, int second) {

    int movedhour=hour << 12;

    int movedminute=minute << 6;

    int bintime=movedhour+movedminute+second;
...
update_watch(appdata_s *ad, watch_time_h watch_time, int ambient)
{
	...
	int decimal=getDecimal(hour, minute, second);

	if (!ambient) {
		//snprintf(watch_text, TEXT_BUF_SIZE, "<align=center>Hello Watch<br/>%02d:%02d:%02d</align>",
		//	hour24, minute, second);

		//int binsize=16-sizeof(getBinary(decimal));

		snprintf(watch_text, TEXT_BUF_SIZE, "<align=center style='background-color=#0000FF;'>binary<br/>%s<br/>decimal<br/>%d</align>",
					getBinary(decimal), decimal);
	} else {
		snprintf(watch_text, TEXT_BUF_SIZE, "<align=center><br/>%s</align>",
			getBinary(decimal));
	}
